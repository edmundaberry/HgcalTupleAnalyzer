...
class HGCHEBRecHit : public Object { 
 ...
 public:
  ...
  HGCHEBRecHit(rechit_type t);
  HGCHEBRecHit(rechit_type t, Collection& c, unsigned int i, int j = 0);
  ...
  double  LambdaCoefficient(){  return 0.48; }  
  double Length() { return std::tanh(Eta()) * LambdaCoefficient(); }
  int Index() { return Layer() - 1 + 30 + 12; }
