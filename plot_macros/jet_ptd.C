...
void jet_ptd( const char * name){
  ...
  char quark_name[100]; sprintf(quark_name, "ptd_quarkPrunedR%sJets", name );
  char gluon_name[100]; sprintf(gluon_name, "ptd_gluonPrunedR%sJets", name );
  ...
  h_quark -> GetXaxis() -> SetTitle("Jet p_{T}D");
  h_quark -> GetYaxis() -> SetTitle("Unit-normalized [A. U.]");

  h_gluon -> GetXaxis() -> SetTitle("Jet p_{T}D");
