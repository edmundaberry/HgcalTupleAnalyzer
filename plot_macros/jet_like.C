...
void jet_like(){
  ...
  TH1F * h_quark = (TH1F*) wjet_file  -> Get(quark_name);
  TH1F * h_gluon = (TH1F*) wjet_file  -> Get(gluon_name);
  ...
  h_quark -> GetXaxis() -> SetRangeUser(0, 1.0);
  h_gluon -> GetXaxis() -> SetRangeUser(0, 1.0);

  h_quark -> GetXaxis() -> SetTitle("Quark jet likelihood");
  h_quark -> GetYaxis() -> SetTitle("Unit-normalized [A. U.]");

  h_gluon -> GetXaxis() -> SetTitle("Quark jet likelihood");
  ...
  h_quark -> SetMaximum(0.3);
  h_gluon -> SetMaximum(0.3);
  ...
  sprintf(quark_name, "Pruned quark jets [R_{cut} = 0.001, Z_{cut} = 0.1]");
  sprintf(gluon_name, "Pruned gluon jets [R_{cut} = 0.001, Z_{cut} = 0.1]");
