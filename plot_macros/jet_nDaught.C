...
void jet_nDaught( const char * name){
  ...
  char quark_name[100]; sprintf(quark_name, "nDaught_quarkPrunedR%sJets", name );
  char gluon_name[100]; sprintf(gluon_name, "nDaught_gluonPrunedR%sJets", name );
  ...
  h_quark -> GetXaxis() -> SetRangeUser(0, 2000);
  h_gluon -> GetXaxis() -> SetRangeUser(0, 2000);

  h_quark -> GetXaxis() -> SetTitle("Jet sub-jet multiplicity");
  h_quark -> GetYaxis() -> SetTitle("Unit-normalized [A. U.]");

  h_gluon -> GetXaxis() -> SetTitle("Jet sub-jet multiplicity");
  ...
  h_quark -> SetMaximum(0.15);
  h_gluon -> SetMaximum(0.15);
  ...
  canvas -> SaveAs("jet_nSubJets.pdf");
