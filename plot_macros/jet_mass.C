...
void jet_mass( const char * name){
  ...
  char gluon_name[100]; sprintf(gluon_name, "mass_gluonPrunedR%sJets", name );
  ...
  h_quark -> GetXaxis() -> SetTitle("Jet mass [MeV]");
  h_quark -> GetYaxis() -> SetTitle("Unit-normalized [A. U.]");

  h_gluon -> GetXaxis() -> SetTitle("Jet mass [MeV]");
  ...
  h_quark -> SetMaximum(0.1);
  h_gluon -> SetMaximum(0.1);
