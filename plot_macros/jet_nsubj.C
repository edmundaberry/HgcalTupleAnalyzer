...
void jet_nsubj( const char * name){
  ...
  char quark_name[100]; sprintf(quark_name, "nsubj_quarkPrunedR%sJets", name );
  char gluon_name[100]; sprintf(gluon_name, "nsubj_gluonPrunedR%sJets", name );
  ...
  h_quark -> GetXaxis() -> SetTitle("Jet nsubjettiness [MeV]");
  h_quark -> GetYaxis() -> SetTitle("Unit-normalized [A. U.]");

  h_gluon -> GetXaxis() -> SetTitle("Jet nsubjettiness [MeV]");
  ...
  h_quark -> SetMaximum(0.2);
  h_gluon -> SetMaximum(0.2);
  ...
  canvas -> SaveAs("jet_nSubJettiness.pdf");
