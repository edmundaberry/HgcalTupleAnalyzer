...
void jet_multi( const char * name){
  ...
  char gluon_name[100]; sprintf(gluon_name, "multi_gluonPrunedR%sJets", name );
  ...
  h_quark -> GetXaxis() -> SetRangeUser(0, 50);
  h_gluon -> GetXaxis() -> SetRangeUser(0, 50);
  ...
  h_quark -> GetXaxis() -> SetTitle("PFCandidate multiplicity");
  h_quark -> GetYaxis() -> SetTitle("Unit-normalized [A. U.]");

  h_gluon -> GetXaxis() -> SetTitle("PFCandidate multiplicity");
  ...
  canvas -> SaveAs("jet_nPFCandidates.pdf");
